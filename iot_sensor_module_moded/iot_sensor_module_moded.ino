...
if(strcmp(topic,inTopic2)==0){
    ...
    Serial.println(tempVoc_base + " " + tempCo2e_base  + " uploaded");
...
void getSgp30Reading(){
  ...
  sgp.setHumidity(getAbsoluteHumidity(temp_c, humidity));
  sgp.IAQmeasure();
  mVoc = sgp.TVOC;
  mCo2e = sgp.eCO2;
  Serial.println(String(mVoc)+ " " +String(mCo2e));
  
}

void sgp30SetBase(){
  sgp.getIAQBaseline(&eCo2_base, &Voc_base);
  Serial.println(String(eCo2_base) + " " + String(Voc_base));
...
void StartABClogic(){
  Wire.beginTransmission(T6713_Address);
 for(int x=0; x<sizeof(cmdStartABC); x++)
 {
    Wire.write(cmdStartABC[x]);
  }
 // end transmission
 Wire.endTransmission();
 // read report of current gas measurement in ppm after delay!
